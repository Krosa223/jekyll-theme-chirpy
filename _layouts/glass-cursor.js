(function () {
  ...
  window.krosaGlassCursor = {
    ...
    /*
     * Read-only state for page interactions such as the archive air-hockey game.
     * x/y are the ACTUAL rendered glass-disc coordinates, not raw pointer coordinates,
     * so collision follows the visible cursor exactly.
     */
    getState: function () {
      return {
        x: currentX,
        y: currentY,
        radius: discRadius,
        visible: Boolean(enabled && hasPointer && activeSurface)
      };
    }
  };
})();
